...
function setDndDefaultActions(actorId) {
  ...
  console.log(
    `${currentActor.name} - made ${currentActor.flags["show-action-remaining"].dnD5eActions.doneDefaultActions.attacks} attacks`
  );
...
Hooks.on("dnd5e.rollAttack", async (item, roll, options) => {
  ...
  const additionalAttacks =
    Number(currentActor.flags["show-action-remaining"].additional_attack) || 0;
  ...
  console.log(
    `Atualizando ataques realizado: ${actorActions.doneDefaultActions.attacks}/${numberOfAttacks}`
  );
...
Hooks.on("preCreateChatMessage", (message, options, userId) => {
  console.log('Hooks.on("preCreateChatMessage)');
  ...
  if (message.rolls && message.flags.dnd5e?.roll?.type === "attack") {
    if (hasAttackAction(actor.id)) {
      return true; // Prevent the message (and roll) from being created
    }
    ui.notifications.error(
      `${actor.name} cannot attack because they have no attack action remaining!`
    );
    return false;
  }
